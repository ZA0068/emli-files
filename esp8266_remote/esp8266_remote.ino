...
#define COUNT_TIME 2000

volatile int button_a_count = 0;
volatile bool activate_pump = false;
volatile unsigned long button_prev_time = 0;
volatile unsigned long count_prev_time = 0;
volatile unsigned long curtime = 0;

// Wifi
#include <ESP8266WiFi.h>
const char* WIFI_SSID = "EMLI_TEAM_24";
const char* WIFI_PASSWORD = "12345678";

// Static IP address
IPAddress IPaddress(192, 168, 20, 222);
...
void loop()
{
  ...
  if (client) {
    ...
    while (client.connected() && millis() - clientConnectTime < CLIENT_TIMEOUT) {
      if (((millis() - button_prev_time) > COUNT_TIME) && (button_a_count == 1))
      {
        activate_pump = true;
        ...
        if (c == '\n') {
          ...
          if (currentLine.length() == 0) {
            ...
            if (currentLine.startsWith("GET /led/red/on")) {
              ...
              sprintf(response_s, "%d, %d", button_a_count, activate_pump);
              button_a_count = 0;
              activate_pump = false;
            }        
